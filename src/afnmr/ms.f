...
c (3i6,i2,3f9.3,4f7.3,i2)
c There is also a short output format: (3i6,i2,3f9.3,i2).
...
c
    ...
    1 continue
           ...
           elseif (arg .eq. '-ibury') then
                ...
                call getarg(iarg,arg)
                read(arg,*) ibury
           elseif (arg .eq. '-atm') then
                iarg = iarg + 1
                call getarg(iarg,arg)
                  open(unit=3,file=arg,status='old',form='formatted')
                  rewind(3)
           elseif (arg .eq. '-rad') then
                iarg = iarg + 1
                call getarg(iarg,arg)
...
c
    2 continue
    3 format('Usage: ms -d <#> -rp <#> -atm <file> -rad <file>')
c
...
100	continue
      read (2,150,end=300) itype(ntype),rtype(ntype),d(ntype)
150	format(i5,2f10.5)
...
c number of unit vectors depends on sphere area and input density
      nua(ntype) = int( (4*pi*rtype(ntype)**2) * d(ntype) )
...
c molecule numbers
c
450	read (3,*,end=750) (co(k,n),k=1,3),iat(n),ias(n),molnum(n)
c
...
c     first the integer coordinate arrays
      do 850 i = 1,natom
         do 800 k = 1,3
            ico(k,i) = int( (co(k,i)-comin(k))/width + 1 )
...
1150	         format(1x,'atom',i6,' dropped (same co as ',i6,')')
               go to 1250
            end if
...
c write out messages
      write (6,1650) natom
1650	format(1x,i6,1x,'atoms')
      write (6,1700) nias(1)
1700	format(1x,i6,1x,'omitted')
      write (6,1750) nias(2)
1750	format(1x,i6,1x,'collision only')
      write (6,1800) nias(3)
1800	format(1x,i6,1x,'surface')
...
3150	         format(1x,'atoms',2i6,' have the same center')
               go to 5150
            end if
            call vnorm(vij,uij)
            call vperp(uij,q)
...
3500	            format(1x,'atoms',3i6,' have concentric circles')
                  go to 4200
               end if
...
5500	         format(3i6,i2,3f9.3,4f7.3,i2)
...
c     first the integer coordinate arrays
      do 5850 iy = 1,ny
         do 5800 k = 1,3
            ico(k,iy) = int( (py(k,iy)-comin(k)-radmax-rp)/width + 1 )
...
c calculate which cube this probe lies in
      ici = int( (pv(1,nv)-comin(1)-radmax-rp)/width + 1 )
      if (ici .lt. 1) ici = 1
      if (ici .gt. maxcub) stop 'cube coordinate too large'
      icj = int( (pv(2,nv)-comin(2)-radmax-rp)/width + 1 )
      if (icj .lt. 1) icj = 1
      if (icj .gt. maxcub) stop 'cube coordinate too large'
      ick = int( (pv(3,nv)-comin(3)-radmax-rp)/width + 1 )
      if (ick .lt. 1) ick = 1
...
8050	format(1x,i6,' contact and ',i6,' saddle and ',
     $   i6,' concave surface points')
      write (6,8100) nshape(1) + nshape(2) + nshape(3)
8100	format(1x,i8,' total surface points')
      write (6,8150) areac,arear,areac+arear
8150	format(1x,'contact area:',f10.2,2x,'reentrant area:',f10.2,
     $   2x,'total area:',f10.2)
      stop
      end
c
