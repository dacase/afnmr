...
!=====================================================================================================================
   ...
   if ( xn*2+1 /= xni .or. yn*2+1 /= yni .or. zn*2+1 /= zni ) then
      write(6,'(a)') "Interpolation Failed because of incorrect dimension"
      stop
   end if
...
!  write(6,'(a)')  '      relax itn & norm ', litn, linorm!, onorm
...
! WJ
            if ( ncyc .gt. 0 .and. litn .ge. ncyc .and. lnorm > onorm ) then
               write(6,'(a,2i6,2f12.4)') "MG FAILED: ncyc, itn, norm, onorm", ncyc, litn, lnorm, onorm
               stop
            end if
!
...
!  write(6,'(a)')  'itn & norm, bdotb1 ', itn, norm, bdotb1
!  write(6,'(a)')  'bcg', bcg
...
!     if (npbopt /= 0) write(6,'(a)')  'max boltzmann factor', maxbf
...
!
   ...
   do while ( (abs(alpcor) > alptol) .and. (alpcnt < alpimx) )
      ...
      alpha = alpha + alpcor
!     write(6,'(a)') 'alpcnt alpha ', alpcnt, alpha
...
!        write(6,'(a)') 'itn, real norm', itn, norm
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if ( mod(itn, itn_checknorm) == 0 ) then
         ...
         norm = sum(abs(phi(1:xmymzm)))
!        write(6,'(a)') 'itn, wsor, norm', itn, wsor, norm
!        if (npbopt /= 0) write(6,'(a)') '   max boltzmann factor', maxbf
