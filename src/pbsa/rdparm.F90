...
subroutine rdparm2(x,ix,ih,ipairs,nf,i_stack)
   ...
   use memory_module
   use parms, only : rk, req, tk, teq, pk, pn, phase, cn1, cn2, solty, asol, &
                     bsol, hbcut, numbnd, numang, nptra, nphb, nimprp, nttyp
...
#  include "box.h"
   ...
   do i=1,nphb
      if( asol(i) /= 0.d0 ) then
         write(6,'(2a)') 'Found a non-zero 10-12 coefficient, but source', &
               ' was not compiled with -DHAS_10_12.'
         write(6,'(2a)') 'If you are using a pre-1994 force field, you', &
