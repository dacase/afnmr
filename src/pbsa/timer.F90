...
subroutine pbtimer_start( num_event )
   ...
   if ( num_event > pbsamaxtime ) then
      write(6,'(a,i6,a,i6)')'index ',num_event,' bigger than pbsamaxtime ',pbsamaxtime
      write(6,'(a)')'attempt to add timer '
      call mexit(6,1)
   else if ( pbtimerswitch(num_event) ) then
      write(6,'(a)')'timer: need to reset the time before start'
      call mexit(6,1)
...
subroutine pbtimer_stop( num_event )
   ...
   if ( num_event > pbsamaxtime )then
      write(6,'(a,i6,a,i6)')'index ',num_event,' bigger than pbsamaxtime ',pbsamaxtime
      write(6,'(a)')'attempt to close timer '
      call mexit(6,1)
   else if ( .not. pbtimerswitch(num_event) ) then
      write(6,'(a)')'timer: need to start the time before stop'
      call mexit(6,1)
   end if
   pbtime_bulletin(num_event) = mytime-pbtime_bulletin(num_event)
...
subroutine pbtimer_summary
   ...
   do i = 1, pbsamaxtime
      if ( .not. pbtimerswitch(i) ) then
         ...
         write(6,'(2a)') "Warning, this timer is not stopped: ",PBTIME_desc(i)
      endif
   enddo
   return
