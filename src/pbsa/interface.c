...
static char *AtomNames_str, *AtomType_str, *ResNames_str;
static int *Ipres_old;
...
REAL_T epbsa(PBOPTSTRUCT_T *opt, PARMSTRUCT_T *prm, REAL_T *x,
	...
	if (freevectors==1 && opt!=NULL) {
      ...
      // recreate the old string style atomnames, resnames, etc. structs:
      AtomNames_str = malloc( 4 * prm->Natom + 1);
      AtomType_str  = malloc( 4 * prm->Natom + 1);
      ResNames_str  = malloc( 4 * prm->Nres + 1);
      for( i = 0; i<prm->Natom; i++ ){
          strncpy( &AtomNames_str[ 4*i ], prm->AtomNames[i], 4 );
          strncpy( &AtomType_str[ 4*i ], prm->AtomType[i], 4 );
      }
      AtomNames_str[4 * prm->Natom] = '\0';
      AtomType_str[4 * prm->Natom] = '\0';

      for( i = 0; i<prm->Nres; i++ ){
          strncpy( &ResNames_str[ 4*i ], prm->ResNames[i], 4 );
      }
      ResNames_str[ 4*prm->Nres ] = '\0';

      // create local copy of Ipres with original indexing scheme:
      Ipres_old = malloc( sizeof(int) * (prm->Nres + 1) );
      for( i = 0; i<prm->Nres + 1; i++ ) Ipres_old[i] = prm->Ipres[i] + 1;

      dummy = (int *) malloc(sizeof(int)*prm->Natom);
      pb_init_(&ifcap,&prm->Natom,&prm->Nres,&prm->Ntypes,&prm->Nbonh,
	     &prm->Nbona,Ipres_old ,prm->Iac,prm->Cno,prm->Iblo,prm->ExclAt,
	     prm->BondHAt1,prm->BondHAt2,prm->BondAt1,prm->BondAt2,
	     dummy,ResNames_str,AtomNames_str,AtomType_str,prm->Charges,
	     prm->Rborn);
	  free(dummy);
	}
	...
	mypb_force_(&prm->Natom,&prm->Nres,&prm->Ntypes,
	     Ipres_old,prm->Iac,prm->Cno,prm->ExclAt,&iteration,
