...
def main(opt):
    ...
    if(opt.verbose):
        for res in range(nres):
            if (mode == 1):
                print ("Initial guess for all residues: Eo = %12.6f V; Hill coefficient = %6.3f; X2 factor = %10.6f" % (xc0,k0/k0_redox,X2(xc0,k0,xvals,allyvals[:,res])))
            else:
                print ("Initial guess for all residues: pKa = %9.5f ; Hill coefficient = %6.3f; X2 factor = %10.6f" % (xc0,k0/k0_ph,X2(xc0,k0,xvals,allyvals[:,res])))
    ...
    for res in range(nres):
        try:
            ...
            if(opt.verbose):
                if (mode == 1):
                    print ("Final results for residue %8s: Eo = %12.6f (+- %.6f ) V; Hill coefficient = %6.3f (+- %.3f ); X2 factor = %10.6f; Temperature = %.2f K" % (resnames[res],xc[res],xc_err[res],k[res]/k0_redox,k_err[res]/k0_redox,X2(xc[res],k[res],xvals,allyvals[:,res]),temp0))
                else:
