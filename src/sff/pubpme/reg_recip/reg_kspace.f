      subroutine reg_kspace(numwats,x,y,z,cg,ewaldcof,
     $       ene,force,frac,recip,
     $       maxexp,mlimit,volume,box,XX,navail,virial)
      implicit none
      double precision x(*),y(*),z(*),cg(*),ewaldcof,box,
     $      ene,force(3,*),frac(3,*),XX(navail),volume,maxexp
      double precision recip(3,3),virial(6)
      integer numwats,mlimit(3),navail
      ...
      do 50 i = 1,numatoms
        ...
        frac(3,i) = z(i)/box
50    continue
      call recip_reg(numatoms,cg,ene,vir,tvir,XX,navail,
...
c--------------------------------------------------------
      subroutine recip_reg(numatoms,charge,eer,vir,tvir,X,navail,
     $      mlimit,volume,recip,force,fraction,ewaldcof,maxexp)
      
      implicit none

      integer  numatoms,mlimit(3)
      real*8   charge(*),eer,vir,tvir(3,3),X(*),volume,recip(3,3)
...
c        write(6,*)'Memory space in the FFT-arrays not large enough '
c        write(6,*)'Provided space: ',navail,' Needed space: ',need
c        stop
      endif
...
C     CALCULATE FORCE ON EACH SITE
                  DO 200 I=1,MAXSIT
                     ...
                     FFXYZ(1,I)=FFXYZ(1,I)+RL*QFORCE
                     FFXYZ(2,I)=FFXYZ(2,I)+RM*QFORCE
                     FFXYZ(3,I)=FFXYZ(3,I)+RN*QFORCE
 200              CONTINUE
C     
C     END VECTOR LOOP
               ENDIF
...
C     CALCULATE EWALD FORCE ARRAYS
      DO 240 I=1,MAXSIT
         FFXYZ(1,I) = 2.0D0*FAC*RVOL*(HI(1)*FFXYZ(1,I)+
     X                HI(2)*FFXYZ(2,I)+HI(3)*FFXYZ(3,I))
         FFXYZ(2,I) = 2.0D0*FAC*RVOL*(HI(4)*FFXYZ(1,I)+
     X                HI(5)*FFXYZ(2,I)+HI(6)*FFXYZ(3,I))
         FFXYZ(3,I) = 2.0D0*FAC*RVOL*(HI(7)*FFXYZ(1,I)+
     X                HI(8)*FFXYZ(2,I)+HI(9)*FFXYZ(3,I))
 240  CONTINUE
C     
