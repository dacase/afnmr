...
PARMSTRUCT_T* rdparm( STRING_T* name )
{
   ...
   prm->AtomNames = (NAME_T *) get(sizeof(NAME_T) * prm->Natom);
   ...
   prm->ResNames = (NAME_T *) get(sizeof(NAME_T) * prm->Nres);
   ...
   prm->AtomType = (NAME_T *) get(sizeof(NAME_T) * prm->Natom);
   prm->AtomTree = (NAME_T *) get(sizeof(NAME_T) * prm->Natom);
   ...
   for (i = 0; i < (prm->Natom / 20 + (prm->Natom % 20 ? 1 : 0)); i++){
      ...
      for( j = 0; j < 20; j++ ){
         if( 20*i + j < prm->Natom ){
            strncpy( prm->AtomNames[20*i + j], NAME_DEFAULT, NAMESIZE );
            strncpy( prm->AtomNames[20*i + j], &pline[ 4*j ], 4 );
         }
      }
   }
   ...
   for (i = 0; i < (prm->Nres / 20 + (prm->Nres % 20 ? 1 : 0)); i++){
      ...
      for( j = 0; j < 20; j++ ){
         if( 20*i + j < prm->Nres ){
            strncpy( prm->ResNames[20*i + j], NAME_DEFAULT, NAMESIZE );
            strncpy( prm->ResNames[20*i + j], &pline[ 4*j ], 4 );
         }
      }
   }
   ...
   for (i = 0; i < prm->Nres; i++) {
      if (get_mytaskid() == 0) {
         fscanf(file, " %d", &prm->Ipres[i]);
          prm->Ipres[i] -= 1;  /* use zero-based atom numbers here! */
                               /* N.B.: need to fix hcp_gb code */
      }
   }
   ...
   for (i = 0; i < (prm->Natom / 20 + (prm->Natom % 20 ? 1 : 0)); i++){
      ...
      for( j = 0; j < 20; j++ ){
         if( 20*i + j < prm->Natom ){
            strncpy( prm->AtomType[20*i + j], NAME_DEFAULT, NAMESIZE );
            strncpy( prm->AtomType[20*i + j], &pline[ 4*j ], 4 );
         }
      }
   }
   ...
   for (i = 0; i < (prm->Natom / 20 + (prm->Natom % 20 ? 1 : 0)); i++){
      ...
      for( j = 0; j < 20; j++ ){
         if( 20*i + j < prm->Natom ){
            strncpy( prm->AtomTree[20*i + j], NAME_DEFAULT, NAMESIZE );
            strncpy( prm->AtomTree[20*i + j], &pline[ 4*j ], 4 );
         }
      }
   }
   ...
   res = 0;
   for (i = 0; i < prm->Natom; i++) {
      if (i == prm->Ipres[res + 1]) /* atom is 1st of next res */
         res++;
      prm->AtomRes[i] = res;
   }
   ...
   if (!prm->IfBox) {
      ...
      if (get_mytaskid() == 0) {
         ...
         fscanf(file, " %lf %lf %lf %lf",
                &prm->Box[3], &prm->Box[0], &prm->Box[1], &prm->Box[2]);
#else
         fscanf(file, " %f %f %f %f",
                &prm->Box[3], &prm->Box[0], &prm->Box[1], &prm->Box[2]);
#endif
      ...
      skipeoln(file);
      if (prm->Iptres)
         prm->Ipatm = prm->Ipres[prm->Iptres];
...
#define BOFFSET 0.09
   if (newparm) {
      ...
      for (i = 0; i < prm->Natom; i++) {
         ...
         atsymb = prm->AtomNames[i][0];        /* first letter atom name */
...
INT_T wrparm( PARMSTRUCT_T* prm, STRING_T* name )
{
   ...
   if (get_mytaskid() == 0) {
      ...
      if (ier >= 0) {
         ...
         for (i = 0; i < prm->Natom; i++) {
            strncpy(tmpchar, prm->AtomNames[i], 4);
            tmpchar[4] = '\0';
         ...
         FortranFormat(20, LBLFORMAT);
         for (i = 0; i < prm->Nres; i++) {
            strncpy(tmpchar, prm->ResNames[i], 4);
            tmpchar[4] = '\0';
            FortranWriteString(tmpchar);
         }
         FortranEndLine();
         ...
         FortranFormat(20, LBLFORMAT);
         for (i = 0; i < prm->Natom; i++) {
            strncpy(tmpchar, prm->AtomType[i], 4);
            tmpchar[4] = '\0';
            FortranWriteString(tmpchar);
         }
         FortranEndLine();
         ...
         FortranFormat(20, LBLFORMAT);
         for (i = 0; i < prm->Natom; i++) {
            strncpy(tmpchar, prm->AtomTree[i], 4);
            tmpchar[4] = '\0';
            FortranWriteString(tmpchar);
         }
         FortranEndLine();
...
{
   PARMSTRUCT_T *newprm;
   NAME_T *AtomNamesbuf, *ResNamesbuf, *AtomTypebuf, *AtomTreebuf;
