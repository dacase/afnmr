...
/*
   ...
   (Note: the eigenvectors are not changed!  Also, need to look at the
   character of the rotational/translational modes more carefully.)
*/
...
*/
   ...
   /*
      ...
      so sort on the norm of what is remaining.
    */
...
void setgam(INT_T natom, REAL_T * gamma, REAL_T * m, REAL_T eta,
            REAL_T hrmax, INT_T ioseen, char name[], REAL_T * a,
            REAL_T * x)
{

/*    
   Setup the friction  matrix, and put into the appropriate place in a
*/
   ...
   if (ioseen == 0) {
      ...
      for (j = 0; j < nr3; j++) {
         ...
         index2 = j / 3;
         gamma[index1] = (factor * expos[index2]) / (m[index2 + 1]);
         index1++;
      }

      /* put friction matrix into the appropriate part of a  */
      /*   for consistency with ioseen > 0, maybe put this into the
           first part of a, then move it later???            */
         ...
         if (expos[iat] != 0.0){
            ...
            for( jat=iat+1; jat<natom; jat++ ){
               if (expos[jat] != 0.0) {
                  ...
                  cons = 1.0 / (etpita*r);
                  a[ix + nr6*jx] = cons * ( 1.0 + delx*delx / r2 );
                  a[iy + nr6*jy] = cons * ( 1.0 + dely*dely / r2 );
                  a[iz + nr6*jz] = cons * ( 1.0 + delz*delz / r2 );
                  a[ix + nr6*jy] = cons * delx * dely / r2;
                  a[ix + nr6*jz] = cons * delx * delz / r2;
                  a[iy + nr6*jz] = cons * dely * delz / r2;
                  a[iy + nr6*jx] = a[ix + nr6*jy];
                  a[iz + nr6*jx] = a[ix + nr6*jz];
                  a[iz + nr6*jy] = a[iy + nr6*jz];
                  
                  /*    ----- rotne prager correction */
                  
                  if (ioseen > 1) {
                     cons = cons / r2 * (expos[iat]*expos[iat] + 
                          expos[jat]*expos[jat]);
                     a[ix+nr6*jx] = a[ix+nr6*jx] + cons*( ont - delx*delx/r2 );
                     a[iy+nr6*jy] = a[iy+nr6*jy] + cons*( ont - dely*dely/r2 );
                     a[iz+nr6*jz] = a[iz+nr6*jz] + cons*( ont - delz*delz/r2 );
                     a[ix+nr6*jy] = a[ix+nr6*jy] - cons*delx*dely/r2;
                     a[ix+nr6*jz] = a[ix+nr6*jz] - cons*delx*delz/r2;
                     a[iy+nr6*jz] = a[iy+nr6*jz] - cons*dely*delz/r2;
                     a[iy+nr6*jx] = a[ix+nr6*jy];
                     a[iz+nr6*jx] = a[ix+nr6*jz];
                     a[iz+nr6*jy] = a[iy+nr6*jz];
                  }
...
/*
    equivalent LAPACK are said to be: 
         DGETRI    determinant and inverse of a general
...
*/
...
#if 0
   /* (re)set the gamma matrix, seems to be needed in the normalization step */
   ij = 0;
   ja = 0;
   do 120 j = 1, 3*natom
      jat = (j+2)/3
   ...
   /* (re)set the gamma matrix, needed in the normalization step */
      ...
      if (expos[jat] == 0.0){
         ...
         for( i=0; i<j; i++ ){
            ...
            if (expos[iat] == 0.0){
               ...
               ia++;
               gamma[ij] = a[ia + nr6*ja] / sqrt (m[iat]*m[jat]);
            }
         }
      }
   }
   free_vector(expos, 0, natom);
}
           ...
           REAL_T * gamma, INT_T * iclass, INT_T natom)
{
   ...
   while (ik < nr6) {
      ...
      if (wi[i] > 0.0000001) {  /* lambda is a complex pair */
      ...
      else if (wr[i] > 0.0) {  /* lambda is positive real */
         ...
         cons = csqrt(lambda / sum);
         for (k = 0; k < (nr6 / 2); k++) {
            zo[i * (nr6) + k] *= creal(cons);
         }
         iclass[ik] = 2;
         ik += 1;
      }

      else if (wr[i] < 0.0) {  /* lambda is negative real */
...
int nmode(REAL_T * x, int n,
          REAL_T(*func) (REAL_T *, REAL_T *, REAL_T *, REAL_T *,
                         ...
                         int *, int *, int *, int *,
                         int *, int *, char *),
          int eigp, int ntrun, REAL_T etau, REAL_T hrmax, int ioseen)
   ...
   *tnmodeHessian += t2 - t1;
   ...
   if (gridim == 1) {
      if (context_PxQ >= 0) {
         ...
         /* Yannick J. Bomble */
         /* compute the Langevin modes if ntrun=3 and regular normal modes 
            otherwise */
         ...
         if (ntrun == 3) {
            ...
            *tnmodeOther += t2 - t1;
            ...
            /*  setgam() sets up the friction matrix and places it into
                the proper location in "a"                                */
            setgam(natom, gamma, m, eta, hrmax, ioseen, name, a, x);
            ...
            *tnmodeSort += t2 - t1;
            ...
            dnorm = vector(0, nr6);
            /*  (should consider making following call optional)  */
            ...
            *tnmodeLan = t2 - t0;
            ...
            /* Make an lmanal-compatible formatted vecs file */
            ...
            if (eigp && mytaskid == 0) {
               ...
               /*  eigenvalues:  */
               for (i = 0; i < 6; i++) {
                  fprintf(vfp, " %4d   %11.4f  %11.4f\n",
                     i + 1, wr[indexsort[i]] * 108.587,
                            wi[indexsort[i]] * 108.587);
               }
               
               for (i = 6; i < nreal; i++) {
                  fprintf(vfp, " %4d   %11.4f  %11.4f  %11.4f\n",
                     i + 1, wr[indexsort[i]] * 108.587,
                            wi[indexsort[i]] * 108.587,
                            dnorm[indexsort[i]]);
               }
               ...
               for (i = 0; i < nr6; i++) {
                  ...
                  for (j = 0; j < nr3; j++) {
                     fprintf(vfp, "  %11.4f", zo[indexsort[i] * nr6 + j]);
                     if (j % 6 == 5)
                        fprintf(vfp, "\n");
                  }
                  if (nr3 % 6 != 0)
