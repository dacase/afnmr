...
!> This module contains various parameters and constants used by the
!! different routines that make up sander.
!! 
...
!! routine.
!! This line needs to go before the implicit none declaration.
!!
...
module constants
  ...
  ! Define C coefficients for associated Legendre polynomials.
  ...
  _REAL_, parameter :: HBAR = 627.509d0 * 0.0241888d-3 * 20.455d0
  ! This is defined as the thermochemical calorie.
  _REAL_, parameter :: J_PER_CAL = 4.184d0
  ! Joules per kilocalorie.
  _REAL_, parameter :: JPKC = J_PER_CAL * 1000.0d0
  ! Boltzmann's constant in J/K.
  _REAL_, parameter :: BOLTZMANN = 1.380658d-23
  ! Avogadro's number.
  _REAL_, parameter :: AVOGADRO = 6.0221367d+23
  ! Boltzmann's constant in internal units.
  _REAL_, parameter :: KB = (BOLTZMANN * AVOGADRO) / JPKC 
  ...
  ! Ratio by which to scale amber charges to get electron charges -
  ! amberchg * oneqscale = electron charges = 1.0 / 18.2223d0
  ...
  ! Charge on a proton in Coulombs.
  _REAL_, parameter :: CHARGE_ON_PROTON = 1.60217733d-19
  ! In meters.
  _REAL_, parameter :: BOHR_RADIUS = 52.9177249d-12
  ...
  !_REAL_, parameter :: BOHRS_TO_A = 0.52917706D0
  ! Bohrs * this = angstroms - Same constants as used in Mopac6 hcore.f.
  !_REAL_, parameter :: BOHRS_TO_A = 0.529177D0
  ! as used in Mopac6 repp.f.
  !_REAL_, parameter :: BOHRS_TO_A = 0.529167D0     
  _REAL_, parameter :: A_TO_BOHRS = 1.0d0 / BOHRS_TO_A
  ! Same constants as used in Mopac6 gover.f.
  !_REAL_, parameter :: A_TO_BOHRS = 1.88976D0
  ! Mopac6 uses 3.5711928576D0 in gover.f for this.
  _REAL_, parameter :: A2_TO_BOHRS2 = A_TO_BOHRS * A_TO_BOHRS 
  _REAL_, parameter :: A3_TO_BOHRS3 = A2_TO_BOHRS2 * A_TO_BOHRS
  _REAL_, parameter :: A4_TO_BOHRS4 = A2_TO_BOHRS2 * A2_TO_BOHRS2

  ! One atomic unit of energy in eV.
  _REAL_, parameter :: ONE_AU_ENERGY = 27.2113962d0
  ! Conversion from Hartrees to Joules.
  _REAL_, parameter :: HARTREE_TO_JOULE = ONE_AU_ENERGY * CHARGE_ON_PROTON
  ! Hartree * Bohrs to Joules * Angstroms.
  _REAL_, parameter :: HART_BOHR_TO_JOULE_A = HARTREE_TO_JOULE * BOHRS_TO_A
  ...
  ! Conversion from AU to EV - not used because we match dynamo v2 below.
  !_REAL_, parameter :: AU_TO_EV = ONE_AU  
  ! Conversion from AU to EV - Same as dynamo v2 uses and Gaussian 98
  ! Note (RCW+MC): more precise would be: 1 a.u. 27.211396 eV Mopac6
  ! uses 27.21D0 in calpar.f, delri.f and repp.f but in ffhpol.f it
  ! uses 27.2107 and in the manual it quotes 27.211
  _REAL_, parameter :: AU_TO_EV = 27.21d0 
  ...
  !_REAL_, parameter :: EV_TO_KCAL = 23.060362D0      
  ! Dynamo parameter
  ! Dynamo's conversion
  _REAL_, parameter :: EV_TO_KCAL = 23.061d0  
  _REAL_, parameter :: KCAL_TO_EV = one / EV_TO_KCAL

  _REAL_, parameter :: AU_TO_KCAL = AU_TO_EV*EV_TO_KCAL !1 hartree.


  ! The following are updated constants from
  ...
  !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  ! Avogadro's number
  _REAL_, parameter :: CODATA08_AVOGADRO       = 6.0221417930d23      
  _REAL_, parameter :: CODATA08_BOHR_RADIUS    = 0.5291772085936d-10
  ! Atomic unit of energy in joules
  _REAL_, parameter :: CODATA08_ONE_AU         = 4.3597439422d-18
  ...
  ! From http://cccbdb.nist.gov/debye.asp (April 12 2011).
  !_REAL_, parameter :: AU_TO_DEBYE = 1.0d0/0.393430
  ...
  ! The BOOK says :
  !
  ! 2Chronicles 4:2 reads thus, 'Also he made a molten sea of ten
  ...
  ! about.'
  ! 
  ! Hence, Pi is exactly equal to three and there is nothing more to
  ! discuss!
  ! 
  ! If you want to use the value of PI defined by 'the BOOK' then
  ! uncomment the following line and comment out the definition
  ! above...
  ...
  _REAL_, parameter :: alpb_alpha = 0.571412d0 
