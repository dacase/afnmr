...
contains
  ...
  function rism3d_closure_excessChemicalPotential(this, huv, cuv) &
    ...
    _REAL_ :: correction(this%solvent%numAtomTypes)
    if (associated(this%kh)) then
    ...
    correction = LJCorrection_excessChemicalPotential(this)
    excessChemicalPotential = excessChemicalPotential + correction
  ...
  function rism3d_closure_kirkwoodBuff (this, guv) result(H)
    ...
    correction = LJCorrection_TCF_int(this)
    H = H * this%grid%voxelVolume + LJCorrection_TCF_int(this)
  ...
  function rism3d_closure_DCFintegral (this,cuv) result(C)
    ...
    correction = LJCorrection_DCF_int(this)
    C = C*this%grid%voxelVolume + correction
...
!!$    end function rism3d_closure_aDCFintegral
  ...
  subroutine rism3d_closure_force(this, ff, guv, periodicPotential)
    ...
    integer :: atom
    integer :: atomRange
!    _REAL_ :: tmpff(3,this%solute%numAtoms)
!    _REAL_ :: DEL
    integer :: k, iat
!    DEL = 1e-4
  ...
  !> Lennard-Jones truncation correction for the excessChemicalPotential.
  !! @param[in] this The closure object.
  !! @return Correction for each solvent site.
  function LJCorrection_excessChemicalPotential(this) result (correction)
    use constants, only : pi
    implicit none
    type(rism3d_closure), intent(in):: this !< Closure object with computed solution.
    _REAL_ :: correction(this%solvent%numAtomTypes)
    integer :: iu, iv
    _REAL_ :: tmp(this%solvent%numAtomTypes)
    _REAL_ :: cor_c(this%solvent%numAtomTypes), cor_hDc(this%solvent%numAtomTypes)
    correction = 0d0

    ! return
    
    !The correction is composed of three parts: 1) the change in potential
    !energy due to the truncation, 2) the charge mulitplied by the long-range
    !asymptotics, and 3) the contribution of the closure.

    if (all(this%potential%ljCutoffs2 == HUGE(1d0)) .or. .not. this%potential%applyLJCorrection) then
       return
    end if

    cor_c = - LJCorrection_DCF_int(this)

    

    ! ! Ingore this correction.  It is extremely small and not worth the effort.
    ! ! 2) Integration over the change in potential energy * TCF long-range
    ! ! asymptotics
    cor_hDc = 0d0
    ! if (this%solvent%ionic .and. this%solute%charged) then
    !    cor_hDc =  - 0.5*rism3d_potential_int_ionic(this)
    ! end if
    ! write(6,*) 'u_tail c+h*dc correction', correction
    
    correction = this%solvent%density*(cor_c + cor_hDc)

    ! write(6,*) 'cor_c', cor_c
    ! write(6,*) 'cor_hDc', cor_hDc
    ! write(6,*) 'cor_tot', correction

    
    ! 3) Closure
    ! No correction for the current closures
  end function LJCorrection_excessChemicalPotential

  !> Lennard-Jones truncation correction for the TCF.
  !! @param[in] this The closure object.
  !! @return Correction for each solvent site.
  function LJCorrection_TCF_int(this) result (correction)
    use rism_util, only : polynomialInterpolation
    implicit none
    type(rism3d_closure), intent(in):: this !< Closure object with computed solution.
    _REAL_ :: correction(this%solvent%numAtomTypes)
    _REAL_ :: ulj(this%solvent%numAtomTypes)
    ! _REAL_ :: xvv, err
    integer :: iv1, iv2
    ! integer :: order = 5
    correction = 0d0
    ...
    ulj = LJCorrection_DCF_int(this)

    do iv1 = 1, this%solvent%numAtomTypes
       do iv2 = 1, this%solvent%numAtomTypes
          ! We should be extrapolating xvv to 0 but this doesn't give
          ! a good result.  A different method may work better.  The
          ! error in using ik=1 value should be smaller, around 1e-4
          ! or 1e-5.
          
          ! call polynomialInterpolation(&
          !      this%solvent%waveNumbers(2:1+order), &
          !      this%solvent%xvv(1:order,iv2,iv1), order,&
          !      0d0, xvv, err)
          ! write(6,*) 'interp', iv1, iv2
          ! write(6,*) 0d0,this%solvent%waveNumbers(2:1+5)
          ! write(6,*) xvv, this%solvent%xvv(1:5,iv2,iv1)
          correction(iv1) = correction(iv1) + ulj(iv2)*this%solvent%xvv(1,iv2,iv1)
       end do
    end do
    ! Don't divided by the number of processes.  This is already done
    ! in DCF_int()
    ! correction = correction/this%grid%mpisize
  end function LJCorrection_TCF_int
  
  !> Lennard-Jones truncation correction for the DCF. This correction
  !> is distributed across parallel processes
  !! @param[in] this The closure object.
  !! @return Correction for each solvent site.
  function LJCorrection_DCF_int(this) result (correction)
    use constants, only : pi
    implicit none
    type(rism3d_closure), intent(in):: this !< Closure object with computed solution.
    _REAL_ :: correction(this%solvent%numAtomTypes)
    ...
    !The correction is the change in potential
    !energy due to the truncation
    if (all(this%potential%ljCutoffs2 == HUGE(1d0)) .or. .not. this%potential%applyLJCorrection) then
       return
    end if
    
    ! 1) Integration over the change in potential energy
    do iv = 1, this%solvent%numAtomTypes
       do iu = 1, this%solute%numAtoms
          correction(iv) = correction(iv) - &
               (1d0/3d0 * this%potential%ljAUV(iu,iv) / this%potential%ljCutoffs2(iu,iv)**(4.5d0) &
               - this%potential%ljBUV(iu,iv) / this%potential%ljCutoffs2(iu,iv)**(1.5d0) )
       end do
    end do
    correction = correction * 4d0/3d0*PI
    correction = correction/this%grid%mpisize
  end function LJCorrection_DCF_Int

  !> Calculates the ionic solvent component of the long-range correction to
  !! excess chemical potential. This is the TCF long-range asymptotics
  !! multiplied by the LJ potential beyond the cuttoff.
  !!
  !! Note that this correction is very small generally not worth computing.
  !!
  !! @param[in] this The closure object.
  !! @return Integral of tcfLongRangeAsympR*ulj(r>r_cut) for each solvent site.
  function LJCorrection_int_ionic(this) result(correction)
    use rism_util, only : gaussquad_laguerre, gaussquad_legendre, spherical_bessel_j0
    use constants, only : pi, fourpi, AMBER_ELECTROSTATIC, KB
    implicit none
    type(rism3d_closure), intent(in):: this !< Closure object with computed solution.
    integer, parameter :: Nintmx = 512
    _REAL_ :: dx
    integer :: ik, iv, iua, iub
    _REAL_ :: argum(Nintmx), weight(Nintmx), sub,dsub
    _REAL_ :: smear2_4, xappa2
    _REAL_ :: hlr
    _REAL_ :: correction(this%solvent%numAtomTypes), correction_tmp
    _REAL_, external :: dsi
    integer, parameter :: limit=1024
    _REAL_ :: abserr, alist(limit),blist(limit),rlist(limit),elist(limit)
    integer :: neval,ier, iord(limit),last
    ! Set Gaussian charge smearing coefficients.
    smear2_4=this%potential%chargeSmear**2 / 4d0
    xappa2 = this%solvent%xappa**2

    correction = 0
    ! for each solvent site
    do iv = 1, this%solvent%numAtomTypes
       ! skip neutral molecules
       if (this%solvent%charge_sp(iv) == 0) then
          cycle
       end if
          ! for each solute atom a
          do iua = 1, this%solute%numAtoms
             ! for each solute atom b
             do iub = 1, this%solute%numAtoms
                ! compute the distance
                dx = sqrt(sum((this%solute%position(:, iua) - this%solute%position(:, iub))**2))
                call dqagie(integrand,0d0,1,-1,1d-6,limit,correction_tmp,&
                     abserr, neval,ier,alist,blist,rlist,elist,iord,last)
                correction(iv) = correction(iv) + correction_tmp
                
             end do
          end do
    end do
  contains
    _REAL_ function integrand(k)
      implicit none
      _REAL_, intent(in) :: k
      _REAL_ :: hlr
      hlr = this%solute%charge(iua) * exp(-smear2_4 * k**2) / (k**2 + xappa2)
      integrand = rism3D_potential_ulj_gt_cut_k(this%potential,k,iub,iv) &
           ! sin(k*dist)*k/dist
           * spherical_bessel_j0(k*dx)*k**2 &
           * hlr
    end function integrand

  end function LJCorrection_int_ionic
...
! Numerical, finite difference way to check force computation.
  subroutine rism3d_checkForceNumDeriv(this, ff, guv, ljTolerance)
    ...
    use rism_util, only : checksum
    implicit none
    type(rism3d_closure), intent(inout) :: this !> potential object.
