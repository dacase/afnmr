    ...
    for (i = 0; i < iVarArrayElementCount(uUnit->vaAtoms); i++) {
        ...
        if (!bFoundSome) {
            ...
            SortByInteger((GENP) PVAI(vaExcludedAtoms, int, iStart), iCount,
                          sizeof(int), (GENP) PVAI(vaExcludedAtoms, int,
                                                   iStart), TRUE);
        }
 ...
 */
    ...
    /* PLUS the number of RESTRAINT bonds/angles/torsions */
     ...
     /*MBONA*/ FortranWriteInt(iBondWithout +
                               iUnitRestraintTypeCount(uUnit, RESTRAINTBOND));
     /*MTHETA*/ FortranWriteInt(iAngleWithout +
                                iUnitRestraintTypeCount(uUnit, RESTRAINTANGLE));
     /*MPHIA*/ FortranWriteInt(iTorsionWithout +
                               iUnitRestraintTypeCount(uUnit,
                                                       RESTRAINTTORSION));
    ...
    /* they will have new parameters */
     ...
     /*MUMBND*/ FortranWriteInt(iParmSetTotalBondParms(uUnit->psParameters) +
                                iUnitRestraintTypeCount(uUnit, RESTRAINTBOND));
     /*MUMANG*/ FortranWriteInt(iParmSetTotalAngleParms(uUnit->psParameters) +
                                iUnitRestraintTypeCount(uUnit, RESTRAINTANGLE));
     /*NPTRA*/ FortranWriteInt(iParmSetTotalTorsionParms(uUnit->psParameters) +
    ...
    if (iVarArrayElementCount(uUnit->vaRestraints))
     ...
     /*NATYP*/ FortranWriteInt(iParmSetTotalAtomParms(uUnit->psParameters));
     /*NHB*/ FortranWriteInt(iParmSetTotalHBondParms(uUnit->psParameters));
     /*IFPERT*/ if (bPert)
        FortranWriteInt(1);
    else
        FortranWriteInt(0);
    ...
    for (i = 0; i < iVarArrayElementCount(uUnit->vaTorsions); i++) {
     ...
     /*MDPER*/ FortranWriteInt(iCountTorsionPerturbed - iCountTorsionBoundary);

    /* Save flag for periodic boundary conditions */

     /*IFBOX*/ if (bUnitUseBox(uUnit)) {
    ...
    /* Save the number of atoms in the largest residue */
     ...
     /*NMXRS*/ FortranWriteInt(iMaxAtoms);

    /* Save flag for cap information */

     /*IFCAP*/ if (bUnitUseSolventCap(uUnit))
        FortranWriteInt(1);
    else
        FortranWriteInt(0);

     /*NUMEXTRA*/ iNumExtra = 0;
    for (i = 0; i < iVarArrayElementCount(uUnit->vaAtoms); i++) {
    ...
    FortranFormat(10, INTFORMAT);
    for (i = 0; i < iVarArrayElementCount(uUnit->vaResidues); i++) {
        FortranWriteInt(PVAI(uUnit->vaResidues, SAVERESIDUEt, i)->
                        iAtomStartIndex);
    }
    FortranEndLine();
 ...
 */
    ...
    for (i = 0; i < iVarArrayElementCount(uUnit->vaAngles); i++) {
        ...
        if (bPERT_ANGLE(bPert, aA, aB, aD))
            continue;
        if (!
            (iAtomElement(aA) == HYDROGEN || iAtomElement(aB) == HYDROGEN
             || iAtomElement(aD) == HYDROGEN)) {
            FortranWriteInt(AMBERINDEX(saPAngle->iAtom1));
            FortranWriteInt(AMBERINDEX(saPAngle->iAtom2));
            FortranWriteInt(AMBERINDEX(saPAngle->iAtom3));
    ...
    for (i = 0; i < iVarArrayElementCount(uUnit->vaAtoms); i++) {
        ...
        if (GDefaults.iGBparm < 3 || GDefaults.iGBparm == 6
                        ...
                        if (aAtomA[0].iAtomicNumber == 7) {
                            ...
                            if (GDefaults.iGBparm == 8) {
                                ...
                                if (!strcmp(cPTemp, "ARG")
                                    &&
                                    !(strncmp
                                      (sAtomName(saPAtom->aAtom), "HH", 2)
                                      && strcmp(sAtomName(saPAtom->aAtom),
                                                "HE"))) {
                                    dGBrad = 1.17;
                                }
                            }
            ...
            case 6:
                 ...
                 *              --JMS 11/4/2013
                 */
                if (strncmp(sType, "C1", 2) && strncmp(sType, "C2", 2)
                    && strncmp(sType, "C3", 2))
                    dGBrad = 1.7;
            ...
            default:
                dGBrad = 1.50;
                break;          /* Radii from J. Phys. Chem. 1994, 98, 1978-1988 */
            }
        }
    ...
    if (bPert) {
        ...
        for (i = 0; i < iVarArrayElementCount(uUnit->vaAtoms); i++) {
            ...
            if (GDefaults.iGibbs) {
                ...
                FortranWriteDouble(ELECTRONTOKCAL *
                                   (dAtomCharge
                                    (PVAI(uUnit->vaAtoms, SAVEATOMt, i)->
                                     aAtom) +
                                    dAtomPertCharge(PVAI
                                                    (uUnit->vaAtoms, SAVEATOMt,
                                                     i)->aAtom)));
    ...
    if (bPolar) {
        ...
        if (bPert) {
            ...
            for (i = 0; i < iMax; i++, saPAtom++) {
                ...
                if (bTmp) {
                    iIndex =
                        iParmSetFindAtom(uUnit->psParameters,
                                         saPAtom->sPertType);
                    iPertTot++;
                } else
                    iIndex =
                        iParmSetFindAtom(uUnit->psParameters, saPAtom->sType);
                ParmSetAtom(uUnit->psParameters, iIndex, sType, &dMass, &dPolar,
                            &dEpsilon, &dRStar, &dEpsilon14, &dRStar14,
                            &dScreenF, &iElement, &iHybridization, sDesc);
                if (dPolar == -1.0) {
                    dPolar = 0.0;
                    if (bTmp)
